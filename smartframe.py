# -*- coding: utf-8 -*-
"""
Created on Fri Jan 14 18:13:59 2022
...
smartframe = pd.read_csv(r'test.csv',header=[0])
smartframe['Date'] = pd.to_datetime(smartframe['Date'])
smartframe = smartframe.sort_values(by="Date")
smartframe['Year'] = smartframe['Date'].dt.year

data_score = smartframe.groupby("Year", as_index=False)\
    .agg(('count', 'mean'))\
    .reset_index()
...
data_score = smartframe.groupby("Location", as_index=False)\
    .agg(('count', 'mean'))\
    .reset_index()
data_score.plot.bar(x="Location", y='Star', rot=70, title="Average Score");

smartframe['Length'] = smartframe['Review'].apply(len)

smartframe.plot.scatter(x = 'Length', y = 'Star')
plt.show()

smartframe['Anonymous'] = smartframe['AUTHOR'].str.contains('Anonymous', regex=False)
#smartframe['Anonymous'] = pd.np.where(smartframe.AUTHOR.str.contains("Anonymous"), 1)

anonframe = smartframe.groupby("Anonymous").agg({'RATING': 'mean'}).reset_index()
...
df = pd.read_csv(r'test.csv',header=[0])
df.columns=['REVIEW','RATING','DATE','LOCATION']

...
"""
# -*- coding: utf-8 -*-
Created on Thu Jan  6 16:04:19 2022
...
ioframe = pd.read_csv(r'reviews_ioCombined.csv',header=[0])

ioframe['Anonymous'] = ioframe['AUTHOR'].str.contains('Anonymous', regex=False)
#ioframe['Anonymous'] = pd.np.where(ioframe.AUTHOR.str.contains("Anonymous"), 1)

anonframe = ioframe.groupby("Anonymous").agg({'RATING': 'mean'}).reset_index()
...
ioframe['Month']= ioframe['TIME'].apply(lambda x: '0' if 'week' in x else x)
ioframe['Month']= ioframe['Month'].apply(lambda x: '0' if 'hour' in x else x)
ioframe['Month']= ioframe['Month'].apply(lambda x: '0' if 'day' in x else x)
discard = ["year"]
ioframe = ioframe[~ioframe['Month'].str.contains('|'.join(discard))]
ioframe['Month'] = ioframe['Month'].str.extract('(\d+)', expand=False)
ioframe['Month'] = pd.to_numeric(ioframe['Month'])

ioaggregate = ioframe.groupby('Month').agg({'RATING': 'mean'}).reset_index()

y = ioaggregate.RATING
x = ioaggregate.Month
ioaggregate.plot.scatter(x = 'Month', y = 'RATING')
plt.show()

ioframe['Length'] = ioframe['COMMENT'].apply(len)

x = ioframe.Length
y = ioframe.RATING
ioframe.plot.scatter(x = 'Length', y = 'RATING')
plt.show()
...
df = ioframe
df.columns=['AUTHOR', 'REVIEW', 'RATING', 'DATE', 'Anonymous', 'Month', 'Length']

# -*- coding: utf-8 -*-
"""
Created on Fri Jan 14 17:44:56 2022
...
fefoframe = pd.read_csv(r'test.csv',header=[0])
fefoframe['Date'] = pd.to_datetime(fefoframe['Date'])
fefoframe = fefoframe.sort_values(by="Date")
fefoframe['Year'] = fefoframe['Date'].dt.year

data_score = fefoframe.groupby("Year", as_index=False)\
    .agg(('count', 'mean'))\
    .reset_index()
...
data_score = fefoframe.groupby("Location", as_index=False)\
    .agg(('count', 'mean'))\
    .reset_index()
data_score.plot.bar(x="Location", y='Star', rot=70, title="Average Score");

fefoframe['Length'] = fefoframe['Review'].apply(len)

fefoframe.plot.scatter(x = 'Length', y = 'Star')
plt.show()

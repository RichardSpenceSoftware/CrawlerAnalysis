# -*- coding: utf-8 -*-
"""
Created on Tue Jan 11 09:24:51 2022
...
pointer.execute("SELECT * FROM TrustPilot")
trustframe = pointer.fetchall()
trustframe = DataFrame(trustframe,columns =['Review Id','Review','Star','Date','Location','Reviews By User','Company'])
trustframe["Star"] = pd.to_numeric(trustframe["Star"])
trustframe['DATE'] = pd.to_datetime(trustframe['Date'])
trustframe = trustframe.sort_values(by="DATE").reset_index(drop=True)
trustframe['Year'] = trustframe['DATE'].dt.month

data_score = trustframe.groupby("Year", as_index=False)\
    .agg(('count', 'mean'))\
    .reset_index()
...
#trustframe['Author']=['Anonymous','Steve','Amanda']


data_score = trustframe.groupby("Location", as_index=False)\
    .agg(('count', 'mean'))\
    .reset_index()
data_score.plot.bar(x="Location", y='Star', rot=70, title="Average Score");
columnName = list(trustframe.columns.values)

trustframe['Length'] = trustframe['Review'].apply(len)

trustframe.plot.scatter(x = 'Length', y = 'Star')
plt.show()

#trustframe['Anonymous'] = trustframe['Author'].str.contains('Anonymous', regex=False)
#trustframe['Anonymous'] = pd.np.where(trustframe.AUTHOR.str.contains("Anonymous"), 1)

#anonframe = trustframe.groupby("Anonymous").agg({'Star Rating': 'mean'}).reset_index()
...
#anonframe.plot.bar(x="Anonymous", y="Star Rating", rot=70, title="Average Score");
...
df = trustframe
df.columns=['REVIEW ID','REVIEW','RATING','DATE','lOCATION','REVIEWS BY USER','COMPANY','Date','YEAR','LENGTH']
df['DATE']=['13 Jan 2020','13 Jan 2020','13 Jan 2020']
df['REVIEW']=['hello','hello','hello']
df.drop(['Date','YEAR','LOCATION','AUTHOR','LENGTH','ANONYMOUS'], axis = 1)
...
df.to_sql('TrustPiloUpdated', engine, index=False)

# -*- coding: utf-8 -*-
"""
Created on Thu Jan 13 16:56:50 2022
...
twitterframe = pd.read_csv(r'TWITTER4OUTPUT.csv',header=[0])
twitterframe = twitterframe.drop(labels=[0,1,2,3,4,5], axis=0)
twitterframe['DATE'] =  twitterframe['DATE'].astype(str) + '2021'
twitterframe['Date'] = pd.to_datetime(twitterframe['DATE'])

twitterframe['Day'] = twitterframe['Date'].dt.day


twitterframe2 = twitterframe.groupby('Day').agg({'RETWEETS': 'sum','REPLIES': 'sum'}).reset_index()

plt.plot(twitterframe2['Day'], twitterframe2['RETWEETS'])
plt.plot(twitterframe2['Day'], twitterframe2['RETWEETS'])
...
df = twitterframe
df.columns=['AUTHOR', 'DATE', 'REVIEW', 'RATING', 'RETWEETS', ' LIKES', 'Date', 'Day']

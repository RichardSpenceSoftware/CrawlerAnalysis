# -*- coding: utf-8 -*-
"""
Created on Wed Jan  5 15:55:36 2022
...
pointer.execute("SELECT * FROM hub")
hubframe = pointer.fetchall()
hubframe = DataFrame(hubframe,columns =['Review','Star Rating','Date','Type of Business','Company'])

hubframe['Star Rating Cut'] = hubframe['Star Rating'].apply(lambda x: x[:2])
hubframe['DATE'] = pd.to_datetime(hubframe['Date'])
hubframe['Year'] = hubframe['DATE'].dt.month
hubframe['Star Rating Cut'] = pd.to_numeric(hubframe['Star Rating Cut'])

data_score = hubframe.groupby("Year", as_index=False)\
    .agg(('count', 'mean'))\
    .reset_index()

plt.plot(data_score['Year'], data_score['Star Rating Cut']['mean'])
plt.title('Average Star Rating per Year')
plt.xlabel('DATE')
plt.ylabel('Star Rating Cut')
plt.axhline(data_score["Star Rating Cut"]['mean'].mean(), color='green', linestyle='--')
plt.show()

hubframe2 = hubframe.groupby("Type of Business").agg({'Star Rating Cut': 'mean'}).reset_index()
transposed = hubframe2.T
...
hubframe2.plot.bar(x="Type of Business", y="Star Rating Cut", rot=70, title="Average Score");
...
df = hubframe
df.columns=['REVIEW', 'LONG RATING', 'DATE', 'BUSINESS', 'RATING','Date', 'YEAR']
